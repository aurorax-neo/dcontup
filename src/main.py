...
def stop_and_remove_container(_pandora: dcontup, _cg: CONFIG, _image_tag: str):
    if _pandora.is_container_not_exist():
        ...
        return True
    # 停止并删除容器提示
    logger.info(f'stop and remove container using {_cg["DOCKER_IMAGE"]["image_name"]}:{_image_tag}')
    # 停止并删除容器
    isStopAndRemove = _pandora.stop_and_remove_container()
    if isStopAndRemove:
        logger.info(f'stop and remove container using {_cg["DOCKER_IMAGE"]["image_name"]}:{_image_tag} success!')
        return True
    logger.info(f'stop and remove container using {_cg["DOCKER_IMAGE"]["image_name"]}:{_image_tag} failed!')
    return False
...
def deploy_new_container(_pandora: dcontup, _cg: CONFIG, _image_tag: str):
    # 部署新容器提示
    logger.info(f'deploy new container using {_cg["DOCKER_IMAGE"]["image_name"]}:{_image_tag}')
    # 部署新容器
    isDeploy = _pandora.deploy_new_container(_image_tag)
    if isDeploy:
        logger.info(f'deploy new container using {_cg["DOCKER_IMAGE"]["image_name"]}:{_image_tag} success!')
        return True
    logger.info(f'deploy new container using {_cg["DOCKER_IMAGE"]["image_name"]}:{_image_tag} failed!')
    return False
